...
public class SeedStrawberry extends Seed {


	public SeedStrawberry(EntityBluePrint blueprint, Level level, int x, int y) {
		super(blueprint, level, x, y, 3250);
							...
							if (entity.equals(this)) {
								if (input.lmouse.isClicked()) {
									if (!cropped && growState == 5) {
										...
										int randomizer = Misc.randomInteger(1, 5);
										
										for (int i = 0; i < randomizer; i++) {
											int x = Misc.randomInteger(-8, 8);
											int y = Misc.randomInteger(-8, 0);
											
											Objects.addInstanceToLevel(Objects.ITEM_STRAWBERRY, level, (int) position.x + x, (int) position.y + y);
										}
									}
								}
							}
