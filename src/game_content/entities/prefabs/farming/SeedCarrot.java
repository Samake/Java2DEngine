...
public class SeedCarrot extends Seed {
	...
	public SeedCarrot(EntityBluePrint blueprint, Level level, int x, int y) {
		super(blueprint, level, x, y, 3000);
								...
								if (input.lmouse.isClicked()) {
									if (!cropped && growState == 5) {
										...
										for (int i = 0; i < randomizer; i++) {
											...
											Objects.addInstanceToLevel(Objects.ITEM_CARROT, level, (int) position.x + x, (int) position.y + y);
										}
									}
								}
							}
