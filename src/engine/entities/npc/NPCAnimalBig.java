...
public class NPCAnimalBig extends NPCCore {
	...
	public NPCAnimalBig(EntityBluePrint bluePrint, Level level, float x, float y, float speed) {
	...
	public void handleAnimation() {
		animation = Animations.ANIMAL_IDLE;
		...
		if (isMoving) {
			// walk up
			if (movingDir == 0) {
				animation = Animations.ANIMAL_WALK_UP;
			// walk down
			} else if (movingDir == 1) {
				animation = Animations.ANIMAL_WALK_DOWN;
			// walk left
			} else if (movingDir == 2) {
				animation = Animations.ANIMAL_WALK_LEFT;
			// walk right
			} else if (movingDir == 3) {
				animation = Animations.ANIMAL_WALK_RIGHT;
			}
		}
