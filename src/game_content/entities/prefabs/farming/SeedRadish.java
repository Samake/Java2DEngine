...
public class SeedRadish extends Seed {
	...
	public SeedRadish(EntityConfig blueprint, Level level, int x, int y) {
		super(blueprint, level, x, y, 4000);
								...
								if (input.lmouse.isClicked()) {
									if (!cropped && growState == 5) {
										...
										for (int i = 0; i < randomizer; i++) {
											...
											Objects.addInstanceToLevel(Objects.ITEM_RADISH, level, (int) position.x + x, (int) position.y + y);
										}
									}
								}
							}
