...
public class SeedAubergine extends Seed {


	public SeedAubergine(EntityBluePrint blueprint, Level level, int x, int y) {
							...
							if (entity.equals(this)) {
								if (input.lmouse.isClicked()) {
									if (!cropped && growState == 5) {
										...
										int randomizer = Misc.randomInteger(1, 2);
										
										for (int i = 0; i < randomizer; i++) {
											int x = Misc.randomInteger(-8, 8);
											int y = Misc.randomInteger(-8, 0);
											
											Objects.addInstanceToLevel(Objects.ITEM_AUBERGINE, level, (int) position.x + x, (int) position.y + y);
										}
									}
								}
							}
